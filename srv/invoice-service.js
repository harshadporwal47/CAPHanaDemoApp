...
module.exports = class InvoiceService extends cds.ApplicationService {
  ...
  async init () {
    ...
    this.on('markAsPaid', Invoices, async (req) => {
      ...
      if (invoice.status === 'PAID')
        return req.error(400, `Invoice '${invoice.invoiceNumber}' is already PAID.`)
      if (invoice.status === 'CANCELLED')
        return req.error(400, `Invoice '${invoice.invoiceNumber}' is CANCELLED and cannot be paid.`)

      await UPDATE(Invoices, ID).set({ status: 'PAID' })
      LOG.info(`Invoice ${invoice.invoiceNumber} marked as PAID`)

      return { message: `Invoice '${invoice.invoiceNumber}' successfully marked as PAID.` }
    })
    ...
    this.on('cancelInvoice', Invoices, async (req) => {
      ...
      if (!invoice)       return req.error(404, `Invoice '${ID}' not found.`)
      if (invoice.status === 'CANCELLED')
        return req.error(400, `Invoice '${invoice.invoiceNumber}' is already CANCELLED.`)
      if (invoice.status === 'PAID')
        return req.error(400, `Invoice '${invoice.invoiceNumber}' is PAID and cannot be cancelled.`)
      ...
      LOG.info(`Invoice ${invoice.invoiceNumber} cancelled. Reason: ${reason ?? 'n/a'}`)

      return { message: `Invoice '${invoice.invoiceNumber}' successfully CANCELLED.` }
    })
    ...
    this.on('getInvoiceSummary', async (req) => {
      ...
      LOG.info(`Invoice summary requested. ${rows.length} status groups returned.`)
      return rows
    })
    ...
    this.on('recalculateInvoiceTotal', async (req) => {
      ...
      const { totalAmount, itemCount } = await this._syncInvoiceTotal(invoiceID)
  ...
  async _syncInvoiceTotal (invoiceID) {
    ...
    const totalAmount = Math.round(
      items.reduce((sum, i) => sum + (i.amount ?? 0), 0) * 100
    ) / 100

    await UPDATE(Invoices, invoiceID).set({ totalAmount })
    LOG.debug(`Synced total for Invoice ${invoiceID}: ${totalAmount} (${itemCount} items)`)
